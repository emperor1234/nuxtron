...
def test_geoptie_end_to_end_automation_suite(client: TestClient) -> None:
    tenant_id = "tenant-geoptie-search-everywhere"

    products = client.get(
        "/search-everywhere/geoptie/products",
        headers=_headers(tenant_id),
    )
    assert products.status_code == 200, products.text
    products_payload = products.json()
    assert products_payload["status"] == "ok"
    assert products_payload["count"] >= 8

    detail = client.get(
        "/search-everywhere/geoptie/products/detail/geo-audit",
        headers=_headers(tenant_id),
    )
    ...
    wiring_apply = client.post(
        "/search-everywhere/geoptie/wiring/apply",
        headers=_headers(tenant_id),
        json={
            "providers": ["OpenAI ChatGPT", "Google Gemini / AI Overview"],
    ...
    assert "geoptie_modules" in full_payload
    assert full_payload.get("production_gate", {}).get("passed") is True

    verification = client.get(
        "/search-everywhere/geoptie/verification-report",
        headers=_headers(tenant_id),
    )
