...
export default function BrightEdgeIntegrationsPage() {
  ...
  async function refreshAll() {
    ...
    try {
      const [checklistData, preflightData] = await Promise.all([
        apiGet<ChecklistResponse>('/search-everywhere/brightedge/provider-checklist', tenantId),
        apiGet<PreflightResponse>('/search-everywhere/brightedge/preflight', tenantId),
      ]);
  ...
  async function buildRemediationBundle() {
    ...
    try {
      const response = await apiSend<RemediationResponse>(
        '/search-everywhere/brightedge/remediation-bundle',
        'POST',
        {
  ...
  async function applyAllMissingWiring() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Failed to apply BrightEdge wiring');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main>
      <section className="hero">
        <p className="eyebrow">BrightEdge Integrations</p>
      ...
      <section className="card" style={{ marginTop: 20 }}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <a
            className="button"
            href="/api/fastapi/search-everywhere/brightedge/remediation-bundle/export?format=csv"
