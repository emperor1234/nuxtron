...
export default function PromptwatchReadinessPage() {
  ...
  async function loadReadiness() {
    ...
    try {
      const [parityData, gateData, vendorData] = await Promise.all([
        apiGet<ParityResponse>('/search-everywhere/promptwatch/parity-audit', tenantId),
        apiGet<GateResponse>('/search-everywhere/promptwatch/production-gate', tenantId),
        apiGet<VendorsResponse>('/search-everywhere/promptwatch/vendors', tenantId),
      ]);
      setParity(parityData);
      setGate(gateData);
      setVendors(vendorData);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to load Promptwatch readiness');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main>
      <section className="hero">
        <p className="eyebrow">Promptwatch Readiness</p>
        <h1>Promptwatch End-to-End Readiness Hub</h1>
        <p>
          Track implementation completeness, integration wiring, vendor coverage, and gate status for the Promptwatch
          automation slice.
        </p>
      </section>
      ...
      <section className="card" style={{ marginTop: 20 }}>
        ...
        >
          <a className="card" href="/seo/promptwatch-automation" style={{ textDecoration: 'none' }}>
            <h3 style={{ marginTop: 0 }}>Automation</h3>
            <p style={{ opacity: 0.8, marginBottom: 0 }}>
              Project runs, product blueprint execution, and production gate controls.
            </p>
          </a>
          <a className="card" href="/seo/promptwatch-integrations" style={{ textDecoration: 'none' }}>
            <h3 style={{ marginTop: 0 }}>Integrations</h3>
            <p style={{ opacity: 0.8, marginBottom: 0 }}>
              Provider checklist, wiring apply, remediation, and export actions.
            </p>
          </a>
          <a className="card" href="/seo/promptwatch-assist" style={{ textDecoration: 'none' }}>
