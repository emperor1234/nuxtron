...
export default function BotifyIntegrationsPage() {
  ...
  async function refreshAll() {
    ...
    try {
      const [checklistData, preflightData] = await Promise.all([
        apiGet<ChecklistResponse>('/search-everywhere/botify/provider-checklist', tenantId),
        apiGet<PreflightResponse>('/search-everywhere/botify/preflight', tenantId),
      ]);
  ...
  async function buildRemediationBundle() {
    ...
    try {
      const response = await apiSend<RemediationResponse>(
        '/search-everywhere/botify/remediation-bundle',
        'POST',
        {
  ...
  return (
    <main>
      <section className="hero">
        <p className="eyebrow">Botify Integrations</p>
      ...
      <section className="card" style={{ marginTop: 20 }}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <a
            className="button"
            href="/api/fastapi/search-everywhere/botify/remediation-bundle/export?format=csv"
