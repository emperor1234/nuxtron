...
export default function BrightEdgeReadinessPage() {
  ...
  async function loadReadiness() {
    ...
    try {
      const [parityData, gateData, vendorData] = await Promise.all([
        apiGet<ParityResponse>('/search-everywhere/brightedge/parity-audit', tenantId),
        apiGet<GateResponse>('/search-everywhere/brightedge/production-gate', tenantId),
        apiGet<VendorsResponse>('/search-everywhere/brightedge/vendors', tenantId),
      ]);
      setParity(parityData);
      setGate(gateData);
      setVendors(vendorData);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to load BrightEdge readiness');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main>
      <section className="hero">
        <p className="eyebrow">BrightEdge Readiness</p>
        <h1>BrightEdge End-to-End Readiness Hub</h1>
        <p>
          Track implementation completeness, integration wiring, vendor coverage, and gate status for the BrightEdge
          automation slice.
        </p>
      </section>
      ...
      <section className="card" style={{ marginTop: 20 }}>
        ...
        >
          <a className="card" href="/seo/brightedge-automation" style={{ textDecoration: 'none' }}>
            <h3 style={{ marginTop: 0 }}>Automation</h3>
            <p style={{ opacity: 0.8, marginBottom: 0 }}>
              Project runs, product blueprint execution, and production gate controls.
            </p>
          </a>
          <a className="card" href="/seo/brightedge-integrations" style={{ textDecoration: 'none' }}>
            <h3 style={{ marginTop: 0 }}>Integrations</h3>
            <p style={{ opacity: 0.8, marginBottom: 0 }}>
              Provider checklist, wiring apply, remediation, and export actions.
            </p>
          </a>
          <a className="card" href="/seo/brightedge-assist" style={{ textDecoration: 'none' }}>
