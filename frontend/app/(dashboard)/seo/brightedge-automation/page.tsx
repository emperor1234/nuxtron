...
type FullAutomationResponse = {
  status: string;
  brightedge_modules?: Record<string, string[]>;
...
export default function BrightEdgeAutomationPage() {
  ...
  async function refreshAll() {
    ...
    try {
      const [projectData, runData, gateData] = await Promise.all([
        apiGet<ProjectsResponse>('/search-everywhere/brightedge/projects', tenantId),
        apiGet<RunsResponse>('/search-everywhere/brightedge/runs', tenantId),
        apiGet<GateResponse>('/search-everywhere/brightedge/production-gate', tenantId),
      ]);
      setProjects(projectData.projects || []);
      setRuns(runData.runs || []);
      setGate(gateData);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to load BrightEdge automation state');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function createDemoProject() {
    ...
    try {
      await apiSend(
        '/search-everywhere/brightedge/projects',
        'POST',
        {
  ...
  async function runLatestProject() {
    if (!projects.length) {
      setError('Create a BrightEdge project first.');
      return;
    }
    setLoading(true);
    setError('');
    try {
      ...
      await apiSend(
        `/search-everywhere/brightedge/projects/${projectId}/run`,
        'POST',
        {
  ...
  async function runFullBlueprint() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Failed to run BrightEdge automation blueprint');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main>
      <section className="hero">
        <p className="eyebrow">BrightEdge Automation</p>
        <h1>BrightEdge Run And Product Control</h1>
        <p>
          Manage projects, execution runs, and product blueprints from the same API surface exposed by the backend
          BrightEdge automation suite.
        </p>
      </section>
