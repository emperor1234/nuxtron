...
export default function YextReadinessPage() {
  ...
  async function loadReadiness() {
    ...
    try {
      const [parityData, gateData, vendorData] = await Promise.all([
        apiGet<ParityResponse>('/search-everywhere/yext/parity-audit', tenantId),
        apiGet<GateResponse>('/search-everywhere/yext/production-gate', tenantId),
        apiGet<VendorsResponse>('/search-everywhere/yext/vendors', tenantId),
      ]);
      setParity(parityData);
      setGate(gateData);
      setVendors(vendorData);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to load Yext readiness');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main>
      <section className="hero">
        <p className="eyebrow">Yext Readiness</p>
        <h1>Yext End-to-End Readiness Hub</h1>
        <p>
          Track implementation completeness, integration wiring, vendor coverage, and gate status for the Yext
          automation slice.
        </p>
      </section>
