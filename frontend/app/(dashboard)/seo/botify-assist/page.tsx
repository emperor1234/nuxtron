...
export default function BotifyAssistPage() {
  const [tenantId] = useState(DEFAULT_TENANT_ID);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [prompt, setPrompt] = useState('Summarize top blockers and recommend activation actions.');
  ...
  async function submitPrompt(event: FormEvent) {
    ...
    try {
      const result = await apiSend<AssistResponse>(
        '/search-everywhere/botify/assist/chat',
        'POST',
        {
          prompt,
      ...
      setError(e instanceof Error ? e.message : 'Failed to run Botify Assist prompt');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main>
      <section className="hero">
        <p className="eyebrow">Botify Assist</p>
        <h1>Assist Prompt Console</h1>
        <p>Run Botify Assist prompts against your latest run, gate, and vendor context.</p>
      </section>
