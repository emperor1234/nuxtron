...
export default function SeoAiReadinessPage() {
  ...
  async function loadReadiness() {
    ...
    try {
      const [parityData, gateData] = await Promise.all([
        apiGet<ParityResponse>('/search-everywhere/seoai/parity-audit', tenantId),
        apiGet<GateResponse>('/search-everywhere/seoai/production-gate', tenantId),
      ]);
      setParity(parityData);
      setGate(gateData);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to load SEO.AI readiness');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main>
      <section className="hero">
        <p className="eyebrow">SEO.AI Readiness</p>
        <h1>SEO.AI End-to-End Readiness Hub</h1>
        <p>Track implementation completeness, integration wiring, and gate status for SEO.AI automation.</p>
      </section>
      ...
      <section className="card" style={{ marginTop: 20 }}>
        ...
        >
          <a className="card" href="/seo/seo-ai-automation" style={{ textDecoration: 'none' }}>
          ...
          <a className="card" href="/seo/seo-ai-integrations" style={{ textDecoration: 'none' }}>
            <h3 style={{ marginTop: 0 }}>Integrations</h3>
            <p style={{ opacity: 0.8, marginBottom: 0 }}>
              Provider checklist, vendors, preflight, remediation, and exports.
            </p>
          </a>
          <a className="card" href="/seo/seo-ai-assist" style={{ textDecoration: 'none' }}>
            <h3 style={{ marginTop: 0 }}>Assist Console</h3>
            <p style={{ opacity: 0.8, marginBottom: 0 }}>Use SEO.AI Assist endpoint with contextual recommendations.</p>
          </a>
        </div>
      </section>
