...
export default function SeoAiAssistPage() {
  const [tenantId] = useState(DEFAULT_TENANT_ID);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [prompt, setPrompt] = useState('Summarize top blockers and recommend SEO.AI execution actions.');
  ...
  async function submitPrompt(event: FormEvent) {
    ...
    try {
      const result = await apiSend<AssistResponse>(
        '/search-everywhere/seoai/assist/chat',
        'POST',
        {
          prompt,
      ...
      setError(e instanceof Error ? e.message : 'Failed to run SEO.AI Assist prompt');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main>
      <section className="hero">
        <p className="eyebrow">SEO.AI Assist</p>
        <h1>Assist Prompt Console</h1>
        <p>Run SEO.AI Assist prompts against your latest run, gate, and vendor context.</p>
      </section>
