...
export default function BotifyAutomationPage() {
  ...
  async function refreshAll() {
    ...
    try {
      const [projectData, runData, gateData] = await Promise.all([
        apiGet<ProjectsResponse>('/search-everywhere/botify/projects', tenantId),
        apiGet<RunsResponse>('/search-everywhere/botify/runs', tenantId),
      ...
      setError(e instanceof Error ? e.message : 'Failed to load Botify automation state');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function createDemoProject() {
    ...
    try {
      await apiSend(
        '/search-everywhere/botify/projects',
        'POST',
        {
          name: 'Botify Automation Project',
          primary_domain: 'nuxtron.ai',
          crawl_urls: ['https://nuxtron.ai', 'https://nuxtron.ai/pricing'],
          analytics_channels: ['organic', 'ai_search', 'paid'],
          competitor_domains: ['semrush.com', 'ahrefs.com'],
          max_urls: 10000,
          enable_activation_workflows: true,
        },
        tenantId
      );
      await refreshAll();
  ...
  async function runLatestProject() {
    if (!projects.length) {
      setError('Create a Botify project first.');
      return;
    }
    setLoading(true);
    setError('');
    try {
      ...
      await apiSend(
        `/search-everywhere/botify/projects/${projectId}/run`,
        'POST',
        {
  ...
  return (
    <main>
      <section className="hero">
        <p className="eyebrow">Botify Automation</p>
        <h1>Botify Run And Schedule Control</h1>
        <p>
          Manage projects and execution runs from the same API surface exposed by the backend Botify automation suite.
        </p>
      </section>
