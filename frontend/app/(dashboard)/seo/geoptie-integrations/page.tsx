...
export default function GeoptieIntegrationsPage() {
  ...
  async function refreshAll() {
    ...
    try {
      const [checklistData, preflightData] = await Promise.all([
        apiGet<ChecklistResponse>('/search-everywhere/geoptie/provider-checklist', tenantId),
        apiGet<PreflightResponse>('/search-everywhere/geoptie/preflight', tenantId),
      ]);
  ...
  async function buildRemediationBundle() {
    ...
    try {
      const response = await apiSend<RemediationResponse>(
        '/search-everywhere/geoptie/remediation-bundle',
        'POST',
        {
  ...
  async function applyAllMissingWiring() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Failed to apply Geoptie wiring');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main>
      <section className="hero">
        <p className="eyebrow">Geoptie Integrations</p>
      ...
      <section className="card" style={{ marginTop: 20 }}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <a
            className="button"
            href="/api/fastapi/search-everywhere/geoptie/remediation-bundle/export?format=csv"
