...
export default function BrightEdgeAssistPage() {
  const [tenantId] = useState(DEFAULT_TENANT_ID);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [prompt, setPrompt] = useState('Summarize top BrightEdge rollout blockers and recommend release actions.');
  ...
  async function submitPrompt(event: FormEvent) {
    ...
    try {
      const result = await apiSend<AssistResponse>(
        '/search-everywhere/brightedge/assist/chat',
        'POST',
        {
          prompt,
      ...
      setError(e instanceof Error ? e.message : 'Failed to run BrightEdge Assist prompt');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main>
      <section className="hero">
        <p className="eyebrow">BrightEdge Assist</p>
        <h1>Assist Prompt Console</h1>
        <p>Run BrightEdge prompts against parity, gate, and vendor context to generate release guidance.</p>
      </section>
