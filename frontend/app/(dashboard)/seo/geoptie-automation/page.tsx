...
type FullAutomationResponse = {
  status: string;
  geoptie_modules?: Record<string, string[]>;
...
export default function GeoptieAutomationPage() {
  ...
  async function refreshAll() {
    ...
    try {
      const [projectData, runData, gateData] = await Promise.all([
        apiGet<ProjectsResponse>('/search-everywhere/geoptie/projects', tenantId),
        apiGet<RunsResponse>('/search-everywhere/geoptie/runs', tenantId),
      ...
      setError(e instanceof Error ? e.message : 'Failed to load Geoptie automation state');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function runLatestProject() {
    if (!projects.length) {
      setError('Create a Geoptie project first.');
      return;
    }
    setLoading(true);
    setError('');
    try {
      ...
      await apiSend(
        `/search-everywhere/geoptie/projects/${projectId}/run`,
        'POST',
        {
          seed_keyword: 'generative engine optimization',
          competitor_domains: ['geoptie.com', 'writesonic.com'],
          max_results: 12,
  ...
  async function runFullBlueprint() {
    ...
    try {
      const result = await apiSend<FullAutomationResponse>(
        '/search-everywhere/geoptie/full-automation',
        'POST',
        {
          seed_keyword: 'generative engine optimization',
      ...
      setError(e instanceof Error ? e.message : 'Failed to run Geoptie automation blueprint');
    } finally {
      setLoading(false);
    }
  }
