...
type FullAutomationResponse = {
  status: string;
  yext_modules?: Record<string, string[]>;
...
export default function YextAutomationPage() {
  ...
  async function refreshAll() {
    ...
    try {
      const [projectData, runData, gateData] = await Promise.all([
        apiGet<ProjectsResponse>('/search-everywhere/yext/projects', tenantId),
        apiGet<RunsResponse>('/search-everywhere/yext/runs', tenantId),
        apiGet<GateResponse>('/search-everywhere/yext/production-gate', tenantId),
      ]);
      setProjects(projectData.projects || []);
      setRuns(runData.runs || []);
      setGate(gateData);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to load Yext automation state');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function createDemoProject() {
    ...
    try {
      await apiSend(
        '/search-everywhere/yext/projects',
        'POST',
        {
          name: 'Yext Automation Project',
          primary_domain: 'nuxtron.ai',
          crawl_urls: ['https://nuxtron.ai', 'https://nuxtron.ai/locations'],
          competitor_domains: ['yext.com', 'uberall.com'],
          max_urls: 10000,
        },
        tenantId
      );
  ...
  async function runLatestProject() {
    if (!projects.length) {
      setError('Create a Yext project first.');
      return;
    }
    setLoading(true);
    setError('');
    try {
      ...
      await apiSend(
        `/search-everywhere/yext/projects/${projectId}/run`,
        'POST',
        {
          seed_keyword: 'brand presence management',
          competitor_domains: ['yext.com', 'uberall.com'],
          max_results: 12,
  ...
  async function runFullBlueprint() {
    ...
    try {
      const result = await apiSend<FullAutomationResponse>(
        '/search-everywhere/yext/full-automation',
        'POST',
        {
          seed_keyword: 'brand presence management',
      ...
      setError(e instanceof Error ? e.message : 'Failed to run Yext automation blueprint');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main>
      <section className="hero">
        <p className="eyebrow">Yext Automation</p>
        <h1>Yext Run And Product Control</h1>
        <p>
          Manage projects, execution runs, and product blueprints from the same API surface exposed by the backend Yext
          automation suite.
        </p>
      </section>
