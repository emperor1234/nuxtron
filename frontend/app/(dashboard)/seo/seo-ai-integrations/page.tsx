...
export default function SeoAiIntegrationsPage() {
  ...
  async function refreshAll() {
    ...
    try {
      const [checklistData, preflightData] = await Promise.all([
        apiGet<ChecklistResponse>('/search-everywhere/seoai/provider-checklist', tenantId),
        apiGet<PreflightResponse>('/search-everywhere/seoai/preflight', tenantId),
      ]);
  ...
  async function buildRemediationBundle() {
    ...
    try {
      const response = await apiSend<RemediationResponse>(
        '/search-everywhere/seoai/remediation-bundle',
        'POST',
        {
  ...
  async function applyAllMissingWiring() {
    ...
    try {
      const response = await apiSend<WiringApplyResponse>(
        '/search-everywhere/seoai/wiring/apply',
        'POST',
        {
          providers: [],
      ...
      setError(e instanceof Error ? e.message : 'Failed to apply SEO.AI wiring');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main>
      <section className="hero">
        <p className="eyebrow">SEO.AI Integrations</p>
      ...
      <section className="card" style={{ marginTop: 20 }}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <a
            className="button"
            href="/api/fastapi/search-everywhere/seoai/remediation-bundle/export?format=csv"
