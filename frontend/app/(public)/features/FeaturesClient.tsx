  ...
  return (
    <>
      <section className="relative px-6 pb-16 pt-40 text-center">
        <Reveal className="mk-container max-w-2xl animate-slide-in-left">
        ...
        <Reveal delay={0.1} className="relative mx-auto mt-14 w-full max-w-4xl px-2 animate-slide-in-right">
          <div
            aria-hidden="true"
            className="absolute inset-x-8 -top-6 h-full rounded-[28px] opacity-30 blur-3xl animate-gradient-flow"
          ...
          <div className="relative overflow-hidden rounded-2xl border border-[#18181814] bg-white shadow-[0_30px_80px_rgba(24,24,24,0.14)]">
            ...
            <Image src="/images/marketing-hub-mockup.png" alt="" width={1536} height={1024} className="w-full transition-opacity duration-700" />
          </div>
        </Reveal>
      </section>
        ...
        <Reveal className="mk-container stagger-children">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            {CAPABILITIES.map((cap, i) => (
              <Reveal key={cap.title} delay={(i % 2) * 0.08} className="mk-card overflow-hidden p-8 group">
                <div className="mb-5 inline-flex w-fit rounded-lg border border-[#18181814] bg-[#466cf314] p-3 text-[#466cf3] transition-transform group-hover:scale-105">
                ...
                <ul className="mb-5 space-y-2 border-t border-[#18181814] pt-5">
                  {cap.points.map((point) => (
                    <li key={point} className="text-sm text-[#8a8d93] transition-colors group-hover:text-[#466cf3]">
                      {point}
                    </li>
                  ))}
                </ul>
                {cap.image ? (
                  <div className="overflow-hidden rounded-xl border border-[#18181814]">
                    ...
                    <Image src={cap.image} alt="" width={1536} height={1024} className="w-full scale-[1.3] origin-top-left transition-transform duration-700 group-hover:scale-[1.35]" />
                  </div>
                ) : (
                  <div
                    className="flex h-28 items-center justify-center rounded-xl border border-[#18181814] transition-all duration-300 group-hover:border-[#466cf3]/50 group-hover:shadow-lg group-hover:shadow-[#466cf3]/20"
                    style={{ background: 'linear-gradient(135deg, #21ccee0f, #466cf314, #f83d690f)' }}
                    aria-hidden="true"
                  >
                    <cap.icon size={36} strokeWidth={1.25} className="text-[#466cf3] opacity-70 transition-transform group-hover:scale-110" />
                  </div>
                )}
              </Reveal>
            ))}
      ...
      <section className="mk-section mk-section-alt">
        <div className="mk-container">
          <Reveal className="mx-auto mb-14 max-w-2xl text-center animate-slide-in-left">
          ...
          <Reveal delay={0.1} className="stagger-children">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              {PRINCIPLES.map((p, i) => (
                <Reveal key={p.label} delay={i * 0.08} className="mk-card p-7 group">
                  <p.icon size={20} strokeWidth={1.75} className="mb-4 text-[#466cf3] transition-transform group-hover:scale-110" aria-hidden="true" />
                  <div className="text-4xl font-bold tracking-tight text-[#181818] [font-family:var(--font-geist-sans)]">
                    <Counter value={p.value} delay={i * 0.15} duration={1.4} className="inline-block" />
                  </div>
