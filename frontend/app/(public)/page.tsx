  ...
  return (
    <>
      ...
      <Hero4
        ...
        badgeText="✦  Now live: autonomous AI agent orchestration"
      ...
      <Reveal delay={0.35} className="mk-fade-up">
          ...
          <div className="mx-auto flex max-w-6xl flex-col items-center gap-6 px-6 md:flex-row md:gap-12">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2.5">
              {CONNECTED.map((item, i) => (
                <Reveal key={item.label} delay={i * 0.06} className="mk-fade-up">
                  <span
                    className="inline-flex items-center gap-1.5 rounded-full border border-[#18181814] bg-white px-3.5 py-1.5 text-sm font-semibold text-[#46484d] hover:shadow-md transition-all duration-300 hover:-translate-y-0.5"
                  >
      ...
      <section className="relative overflow-hidden px-6 py-20" style={{ background: 'linear-gradient(160deg, #eef4ff, #f7fbff)' }}>
        <div className="mk-container grid grid-cols-1 items-center gap-12 lg:grid-cols-2">
          <Reveal className="animate-slide-in-left">
            ...
            <Link href="/features" className="mk-btn-primary mt-8 inline-flex group">
              Explore SEO & AI visibility <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
            </Link>
          </Reveal>
          <Reveal delay={0.1} className="animate-slide-in-right relative mx-auto aspect-square w-full max-w-sm">
      ...
      <section className="relative overflow-hidden px-6 py-20" style={{ background: 'linear-gradient(160deg, #fdeef5, #fff7fb)' }}>
        <div className="mk-container grid grid-cols-1 items-center gap-12 lg:grid-cols-2">
          <Reveal className="animate-slide-in-right order-2 mx-auto w-full max-w-sm lg:order-1">
            <div className="stagger-children space-y-3">
              {['Plan the work', 'Show what will change', 'Wait for your approval', 'Execute & log every step'].map((step, i) => (
                <div key={step} className="mk-card flex items-center gap-3 px-4 py-3 group">
                  <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[#f83d6914] text-xs font-bold text-[#f83d69] transition-all group-hover:scale-110">
                    {i + 1}
                  </span>
          ...
          <Reveal delay={0.1} className="animate-slide-in-left order-1 lg:order-2">
            ...
            <Link href="/features" className="mk-btn-primary mt-8 inline-flex group">
              See how IRA works <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
            </Link>
          </Reveal>
        </div>
      </section>
      ...
      <section className="relative overflow-hidden bg-[#181818] px-6 py-20">
        ...
        <div className="mk-container relative grid grid-cols-1 items-center gap-12 lg:grid-cols-2">
          <Reveal className="animate-slide-in-left">
            ...
            <Link
              href="/contact"
              className="mt-8 inline-flex items-center gap-2 rounded-full border border-white/20 px-7 py-3.5 font-semibold text-white transition-colors hover:bg-white/10 group"
            >
          ...
          <Reveal
            delay={0.1}
            className="animate-slide-in-right relative mx-auto grid w-full max-w-sm gap-3 transition-all"
          ...
          >
            <div style={{ display: 'contents' }} aria-hidden="true">
              {Array.from({ length: 16 }).map((_, i) => (
                <div
                  key={i}
                  className="grid aspect-square place-items-center rounded-xl border border-white/10 transition-all hover:border-white/30 hover:scale-[1.02]"
      ...
      <section className="mk-section">
        <div className="mk-container">
          <Reveal className="mx-auto mb-16 max-w-2xl text-center animate-slide-in-left">
          ...
          <Reveal delay={0.1} className="stagger-children">
            <SolutionsGrid />
          </Reveal>
        </div>
      </section>
      ...
      <section className="mk-section mk-section-alt">
        <div className="mk-container">
          <Reveal className="mx-auto mb-14 max-w-2xl text-center animate-slide-in-right">
            <span className="mk-eyebrow-brand mb-5 inline-flex">Key Tools</span>
            <h2 className="text-4xl font-semibold tracking-tight text-[#181818] sm:text-5xl">
              AI that moves work forward &amp; faster
            </h2>
          </Reveal>
          <Reveal delay={0.1} className="animate-scale-in">
            <KeyToolsTabs />
          </Reveal>
        </div>
      </section>
      ...
      <section className="mk-section">
        <div className="mk-container">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {STATS.map((stat, i) => (
              <Reveal key={stat.label} delay={i * 0.08} className="mk-card p-7 group">
                <div className="mb-4 flex items-center gap-2">
                  <stat.icon size={20} strokeWidth={1.75} className="text-[#466cf3]" aria-hidden="true" />
                </div>
                <div className="text-4xl font-bold tracking-tight text-[#181818] [font-family:var(--font-geist-sans)]">
                  <Counter value={stat.value} delay={i * 0.15} duration={1.4} className="inline-block" />
                </div>
      ...
      <section className="mk-section mk-section-alt text-center">
        <Reveal className="mk-container animate-scale-in">
          ...
          <Link href="/pricing" className="mk-btn-secondary mt-10 inline-flex group">
            See full pricing <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
          </Link>
        </Reveal>
      </section>
      ...
      <section className="mk-section mk-section-alt text-center">
        <Reveal className="mk-container max-w-3xl animate-scale-in">
